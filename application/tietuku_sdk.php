...
class TTKClient{
	 ...
	 * @ignore
	 */
	public $upload_host = "http://up.tietuku.cn/";
	public $host = "http://api.tietuku.cn/v1/";
	/**
	 * Set timeout default.
	 *
	 * @ignore
    ...
    function uploadFromWeb($aid,$fileurl=null){
        ...
        if(is_array($fileurl)){
            ...
            foreach($fileurl as $k=>$v){
                ...
                $d['fileurl']=$v;
                $r=$this->post($url,$d);
                $res_array=json_decode($r,true);
                array_push($ret,$res_array['s_url']);
            }
            return $ret;
        }else{
            $param['deadline'] = time()+$this->timeout;
            ...
            $r=empty($fileurl)?$Token:$this->post($url,$data);
            $res_array=json_decode($r,true);
            return $res_array['s_url'];
        }
    }
	/**
	 * 对接口post数据
